...
public class RenderEffectBloom : MonoBehaviour
{
    ...
    void OnRenderImage(RenderTexture sourceTexture, RenderTexture destTexture)
    {
        if (BloomShader != null)
        {
            ...
            RenderTexture tempBrightPass = RenderTexture.GetTemporary(sourceTexture.width, sourceTexture.height);
            RenderTexture tempBlurPass = RenderTexture.GetTemporary(sourceTexture.width, sourceTexture.height);

            // Blit using bloom shader pass 0 for bright pass ( Graphics.Blit(SOURCE, DESTINATION, MATERIAL, PASS INDEX);)
            Graphics.Blit(sourceTexture, tempBrightPass, ScreenMat, 0);
            ...
            Graphics.Blit(tempBrightPass, tempBlurPass, ScreenMat, 1);
            ...
            Graphics.Blit(tempBlurPass, destTexture, ScreenMat, 2);

            // Release both temp rendertextures
            RenderTexture.ReleaseTemporary(tempBlurPass);
            RenderTexture.ReleaseTemporary(tempBrightPass);
        }
        else
        {

...
Shader "Custom/HillFromTexture"
{
    ...
    _CurrMap("CurrentMap", float) = 0.0
        _HeightMapTex1("HeightMapTexture1", 2D) = "white" {}
    _HeightMapTex2("HeightMapTexture2", 2D) = "white" {}
    _HeightMapTex3("HeightMapTexture3", 2D) = "white" {}
...
#include "UnityCG.cginc"
    ...
    uniform sampler2D _HeightMapTex2;
    uniform sampler2D _HeightMapTex3;
    uniform float _DisplacementAmt;
    uniform float _CurrMap;
    ...
    v2f vert(appdata v)
    {
        ...
        float3 hmVal1 = tex2Dlod(_HeightMapTex1, float4(v.uv, 0, 0)).rgb;
        float3 hmVal2 = tex2Dlod(_HeightMapTex2, float4(v.uv, 0, 0)).rgb;
        float3 hmVal3 = tex2Dlod(_HeightMapTex3, float4(v.uv, 0, 0)).rgb;
        float3 hmVal = hmVal1;


        //BLENDING MOD
        if (_CurrMap > 0 && _CurrMap <= 1)
        {
            hmVal = hmVal1 * (1 - _CurrMap) + hmVal2 * (_CurrMap);
        }
        else if (_CurrMap > 1 && _CurrMap <= 2)
        {
            hmVal = hmVal2 * (2 - _CurrMap) + hmVal3 * (_CurrMap - 1);
        }
        else if (_CurrMap > 2 && _CurrMap <= 3)
        {
            hmVal = hmVal3 * (3 - _CurrMap) + hmVal1 * (_CurrMap - 2);
        }
